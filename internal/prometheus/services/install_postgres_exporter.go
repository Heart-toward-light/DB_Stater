...
// 安装节点 exporter
type InstallPostgresExporter struct {
	prepare *config.PostgresExporterConf
	//consulConfig    *config.ConsulConfig
	Port     int
	basePath string
}

func NewInstallPostgresExporter() *InstallPostgresExporter {
	return &InstallPostgresExporter{
		prepare: &config.PostgresExporterConf{},
	}
}

func (i *InstallPostgresExporter) Run(pre config.PostgresExporterConf, cfgFile string) error {
	// 初始化参数和配置环节
	...
	i.Info(config.PostgresExporter, i.Port)
...
func (i *InstallPostgresExporter) HandlePrepareArgs(pre config.PostgresExporterConf, cfgFile string) error {
	if cfgFile != "" {
...
func (i *InstallPostgresExporter) MergePrepareArgs(pre config.PostgresExporterConf) {
	...
	if pre.PgAddr != "" {
		i.prepare.PgAddr = pre.PgAddr
	}

	if pre.Pass != "" {
		i.prepare.Pass = pre.Pass
	}
}

func (i *InstallPostgresExporter) HandleArgs() {
...
func (i *InstallPostgresExporter) getPackageFullName(app string) string {
...
func (i *InstallPostgresExporter) getServerPath(app string, port int) string {
	return filepath.Join(i.basePath, fmt.Sprintf("%s%d", app, port))
}

func (i *InstallPostgresExporter) getDataPath(app string) string {
	return filepath.Join(i.basePath, app, "data")
}

func (i *InstallPostgresExporter) getServiceFileName(app string, port int) string {
	return fmt.Sprintf("%s%d.service", app, port)
}

func (i *InstallPostgresExporter) getServiceFileFullName(app string, port int) string {
	return filepath.Join(i.getServerPath(app, port), i.getServiceFileName(app, port))
}
func (i *InstallPostgresExporter) Install() error {
	...
	port := i.Port
	logger.Infof("开始安装 Postgres exporter, port:%d \n", port)
	// 创建子目录
	logger.Infof("创建目录\n")
	if err := os.MkdirAll(i.getServerPath(config.PostgresExporter, port), 0755); err != nil {
		return err
	}

	// 解压安装包
	packageFullName := i.getPackageFullName(config.PostgresExporter)
	logger.Infof("解压安装包: %s 到 %s \n", packageFullName, i.getServerPath(config.PostgresExporter, port))
	if err := utils.UntarGz(packageFullName, i.getServerPath(config.PostgresExporter, port)); err != nil {
		return err
	}

	logger.Infof("添加 service 文件\n")
	serviceName := i.getServiceFileName(config.PostgresExporter, port)
	body := fmt.Sprintf(config.PostgresExporterService, i.prepare.Pass, i.prepare.PgAddr, i.basePath, port, port)
...
func (i *InstallPostgresExporter) Info(app string, port int) {
