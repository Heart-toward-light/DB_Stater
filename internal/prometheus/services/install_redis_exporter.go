...
// 安装 redis exporter
type InstallRedisExporter struct {
	prepare *config.RedisExporterConf
	//consulConfig    *config.ConsulConfig
	Port     int
	basePath string
}

func NewInstallRedisExporter() *InstallRedisExporter {
	return &InstallRedisExporter{
		prepare: &config.RedisExporterConf{},
	}
}

func (i *InstallRedisExporter) Run(pre config.RedisExporterConf, cfgFile string) error {
	// 初始化参数和配置环节
	...
	i.Info(config.RedisExporter, i.Port)
...
func (i *InstallRedisExporter) HandlePrepareArgs(pre config.RedisExporterConf, cfgFile string) error {
	if cfgFile != "" {
...
func (i *InstallRedisExporter) MergePrepareArgs(pre config.RedisExporterConf) {
	...
	if pre.RedisAddr != "" {
		i.prepare.RedisAddr = pre.RedisAddr
	}

	if pre.RedisPassword != "" {
		i.prepare.RedisPassword = pre.RedisPassword
	}
}

func (i *InstallRedisExporter) HandleArgs() {
...
func (i *InstallRedisExporter) getPackageFullName(app string) string {
...
func (i *InstallRedisExporter) getServerPath(app string, port int) string {
	return filepath.Join(i.basePath, fmt.Sprintf("%s%d", app, port))
}

func (i *InstallRedisExporter) getDataPath(app string) string {
	return filepath.Join(i.basePath, app, "data")
}

func (i *InstallRedisExporter) getServiceFileName(app string, port int) string {
	return fmt.Sprintf("%s%d.service", app, port)
}

func (i *InstallRedisExporter) getServiceFileFullName(app string, port int) string {
	return filepath.Join(i.getServerPath(app, port), i.getServiceFileName(app, port))
}
func (i *InstallRedisExporter) Install() error {
	...
	port := i.Port
	logger.Infof("开始安装 Redis exporter, port:%d \n", port)
	// 创建子目录
	logger.Infof("创建目录\n")
	if err := os.MkdirAll(i.getServerPath(config.RedisExporter, port), 0755); err != nil {
		return err
	}

	// 解压安装包
	packageFullName := i.getPackageFullName(config.RedisExporter)
	logger.Infof("解压安装包: %s 到 %s \n", packageFullName, i.getServerPath(config.RedisExporter, port))
	if err := utils.UntarGz(packageFullName, i.getServerPath(config.RedisExporter, port)); err != nil {
		return err
	}

	serviceName := i.getServiceFileName(config.RedisExporter, port)
	logger.Infof("添加 service 文件 %s\n", serviceName)
	body := fmt.Sprintf(config.RedisExporterService, i.basePath, port, i.prepare.RedisAddr, i.prepare.RedisPassword, port)
...
func (i *InstallRedisExporter) Info(app string, port int) {
