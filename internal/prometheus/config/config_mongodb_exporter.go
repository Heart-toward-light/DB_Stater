...
// 安装时读取的配置文件
type MongodbExporterConf struct {
	Port            int    `ini:"port" comment:"监听端口，如果没有特殊要求请勿修改"`
	Dir             string `ini:"dir" comment:"数据部署目录，请确认该目录存在，默认为/opt/exporters，如无特殊要求请勿修改"`
	MongodbAddr     string `ini:"mongodb-addr" comment:"mongodb 实例的地址，eg. 127.0.0.1:27017"`
	MongodbPassword string `ini:"mongodb-password" comment:"mongodb 实例的密码"`
	MongodbUser     string `ini:"mongodb-user" comment:"mongodb 实例的用户"`
}

// 确定配置文件位置
func (p *MongodbExporterConf) CfgPath(cfgFile string) (string, error) {
	if cfgFile != "" {
...
func (p *MongodbExporterConf) Load(filename string) error {
...
func (p *MongodbExporterConf) SaveTo(filename string) error {
...
func (p *MongodbExporterConf) InitArgs() {
	logger.Infof("初始化安装参数\n")

	if p.Port == 0 {
		p.Port = utils.RandomPort(DefaultMongodbExporterPort)
	}
...
func (p *MongodbExporterConf) Validator() error {
	logger.Infof("验证参数\n")
...
func (p *MongodbExporterConf) validatePort(port int) error {
	// 端口
