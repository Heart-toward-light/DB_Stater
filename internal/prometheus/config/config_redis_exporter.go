...
// 安装时读取的配置文件
type RedisExporterConf struct {
	Port          int    `ini:"port" comment:"监听端口，如果没有特殊要求请勿修改"`
	Dir           string `ini:"dir" comment:"数据部署目录，请确认该目录存在，默认为/opt/exporters，如无特殊要求请勿修改"`
	RedisAddr     string `ini:"redis-addr" comment:"redis 实例的地址，eg. 127.0.0.1:6379"`
	RedisPassword string `ini:"redis-password" comment:"redis 实例的密码"`
}

// 确定配置文件位置
func (p *RedisExporterConf) CfgPath(cfgFile string) (string, error) {
	if cfgFile != "" {
...
func (p *RedisExporterConf) Load(filename string) error {
...
func (p *RedisExporterConf) SaveTo(filename string) error {
...
func (p *RedisExporterConf) InitArgs() {
	logger.Infof("初始化安装参数\n")

	if p.Port == 0 {
		p.Port = utils.RandomPort(DefaultRedisExporterPort)
	}
...
func (p *RedisExporterConf) Validator() error {
	logger.Infof("验证参数\n")
...
func (p *RedisExporterConf) validatePort(port int) error {
	// 端口
