...
// 安装时读取的配置文件
type PostgresExporterConf struct {
	Port   int    `ini:"port" comment:"监听端口，如果没有特殊要求请勿修改"`
	Dir    string `ini:"dir" comment:"数据部署目录，请确认该目录存在，默认为/opt/exporters，如无特殊要求请勿修改"`
	PgAddr string `ini:"pg_addr" comment:"pg 实例的地址，eg. 127.0.0.1:5432"`
	Pass   string `ini:"pass" comment:"pg 实例的密码"`
}

// 确定配置文件位置
func (p *PostgresExporterConf) CfgPath(cfgFile string) (string, error) {
	if cfgFile != "" {
...
func (p *PostgresExporterConf) Load(filename string) error {
...
func (p *PostgresExporterConf) SaveTo(filename string) error {
...
func (p *PostgresExporterConf) InitArgs() {
	logger.Infof("初始化安装参数\n")

	if p.Port == 0 {
		p.Port = utils.RandomPort(DefaultPostgresExporterPort)
	}
...
func (p *PostgresExporterConf) Validator() error {
	logger.Infof("验证参数\n")
...
func (p *PostgresExporterConf) validatePort(port int) error {
	// 端口
